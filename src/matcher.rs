...
pub fn match_query<'a>(query: &str, matchers: &'a Vec<Matcher>) -> Vec<&'a Matcher<'a>> {
...
fn match_selection<'a>(
    ...
    match selection {
        Selection::Field(field) => matchers.iter().filter(|matcher| matcher.matches_field(field)).collect(),
